...
static void programRunner(GtkWidget *widget, gpointer data) {
    ...
    if ((textfile && cryptfile) && (key = atoi(text)) && key && (encryptFlag || decryptFlag)) {
        if (encryptFlag) {
            char alfabe[127-32];
            alfabet(alfabe);
            ...
            strcat(efile, "/");
            strcat(efile, sfile);
            encrypt(alfabe, key, textfile, efile);
        ...
        if (decryptFlag) {
            ...
            strcat(dfile, "/");
            strcat(dfile, sfile);
            decrypt(alfabe, key, textfile, dfile);
            gtk_label_set_markup(GTK_LABEL(labelSucc), "<span foreground='green'>Success</span>");
...
int main(int argc, char *argv[]) {
    ...
    gtk_window_set_title(GTK_WINDOW(window), "Cezar");
