...
static void programRunner(GtkWidget *widget, gpointer data) {
    if ((textfile && cryptfile) && (encryptFlag || decryptFlag)) {
        char alfabe[127-32];
        alfabet(alfabe);
        unsigned int row = strlen(alfabe);
        unsigned int col = strlen(alfabe);
        char matrix[row][col];
        memset(matrix, 0, sizeof(matrix));
        matrixCreator(row, col, matrix, alfabe);
...
int main(int argc, char *argv[]) {
    ...
    gtk_window_set_title(GTK_WINDOW(window), "Tritemia");
