...
void on_file_selected2(GtkFileChooserButton *filechooserbutton, gpointer user_data) {
    const gchar *filename = gtk_file_chooser_get_filename(GTK_FILE_CHOOSER(filechooserbutton));
    keyfile = (char *)filename;
}

void on_file_selected3(GtkFileChooserButton *filechooserbutton, gpointer user_data) {
    const gchar *filename = gtk_file_chooser_get_filename(GTK_FILE_CHOOSER(filechooserbutton));
    alfabetfile = (char *)filename;
}
...
static void programRunner(GtkWidget *widget, gpointer data) {
    char alfabet[fsize(alfabetfile)];
    cptext(alfabet, fsize(alfabetfile), alfabetfile);

    char key[fsize(keyfile)];
    cptext(key, fsize(keyfile), keyfile);

    if ((textfile && cryptfile) && strcmp(key, "") && strcmp(alfabet, "") && (encryptFlag || decryptFlag)) {
        if (encryptFlag) {
            ...
            encrypt(alfabet, key, textfile, efile);
        ...
        if (decryptFlag) {
            ...
            decrypt(alfabet, key, textfile, dfile);
...
int main(int argc, char *argv[]) {
    GtkWidget *window;
    GtkWidget *file_chooser_button, *file_chooser_button2, *file_chooser_button3;
    ...
    gtk_window_set_title(GTK_WINDOW(window), "Replacement");
    ...
    file_chooser_button2 = gtk_file_chooser_button_new("Select an alfabet file", GTK_FILE_CHOOSER_ACTION_OPEN);
    g_signal_connect(file_chooser_button2, "file-set", G_CALLBACK(on_file_selected2), NULL);

    file_chooser_button3 = gtk_file_chooser_button_new("Select a key file", GTK_FILE_CHOOSER_ACTION_OPEN);
    g_signal_connect(file_chooser_button3, "file-set", G_CALLBACK(on_file_selected3), NULL);
    ...
    gtk_box_pack_start(GTK_BOX(vbox), file_chooser_button2, FALSE, FALSE, 5);
    gtk_box_pack_start(GTK_BOX(vbox), file_chooser_button3, FALSE, FALSE, 5);
